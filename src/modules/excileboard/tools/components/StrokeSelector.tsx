...
type ColorPalette = Record<ColorName, ColorShades>
            ...
            <div className="flex flex-row gap-2">
                ...
                <Popover>
                    <PopoverTrigger asChild>
                        <Button size={"icon-sm"} style={{ backgroundColor: styleManager.strokeColor }}></Button>
                    </PopoverTrigger>
                    <PopoverContent align="start" side="right" sideOffset={25} className="w-full">
                        <PopoverHeader>
                                    ...
                                    {Object.entries(COLOR_PALETTE).map(([name, shades]) => (
                                        <div key={name} className="grid grid-flow-col gap-1">
                                            {shades.map((shade, index) => (
                                                <Button key={`${shade}-${index}`} size={"icon-sm"} style={{ backgroundColor: shade}} className={cn(shade == styleManager.strokeColor && "ring ring-primary ring-offset-1")} onClick={() => handleStrokeSelection(shade)}></Button>
                                            ))}
                                        </div>
                                    ))}
                                </div>
